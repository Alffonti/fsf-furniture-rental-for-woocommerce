...
class phive_booking_core{
    ...
    add_action( 'woocommerce_thankyou', array($this, 'woocommerce_auto_complete_order') ); // Auto Complete all WooCommerce orders.
	}
  ...
  public function woocommerce_auto_complete_order( $order_id ) {
	...
	public function phive_hide_details_from_checkout_and_email($meta)
	{
		...
		{
			$criteria = array(  'key' => __('From','furniture-rental-for-woocommerce') );
			$meta = wp_list_filter( $meta, $criteria, 'NOT' );
		}
		if ($booked_to_exists === true)
		{
			$criteria = array(  'key' => __('To','furniture-rental-for-woocommerce') );
			$meta = wp_list_filter( $meta, $criteria, 'NOT' );
		}
		return $meta;
	}
	...
	*/
	...
	public function phive_added_to_cart_message( $message, $products ){
		...
		if( $is_booking_product ){
			$message = '<a href="'.wc_get_page_permalink( 'cart' ).'" class="button wc-forward">'.__('View cart','furniture-rental-for-woocommerce').'</a> '.__('Booking Done. Please check cart for payment.','furniture-rental-for-woocommerce');
	...
	*/
	...
	function phive_disply_item_booking_infos($item_data, $cart_item){
		...
		if( array_key_exists('phive_book_from_date', $cart_item) && $cart_item['phive_book_from_date'] == $cart_item['phive_book_to_date'] ){
			$item_data[] = array(
				'key'   => __('Booked','furniture-rental-for-woocommerce'),
			...
			if(array_key_exists('phive_book_from_date', $cart_item)){
				$item_data[] = array(
					'key'   => __('Booked from','furniture-rental-for-woocommerce'),
					'value' => phive_booking_ajax_manager::phive_get_date_in_wp_format($cart_item['phive_book_from_date']),
				);
			}
			if(array_key_exists('phive_book_to_date', $cart_item)){
				$item_data[] = array(
					'key'   => __('Booked to','furniture-rental-for-woocommerce'),
	...
	function phive_add_booking_info_order_line_item_meta($item, $cart_item_key, $values, $order){
		...
		if(array_key_exists('phive_book_from_date', $values) && $values['phive_book_from_date']==$values['phive_book_to_date']){
			$item->add_meta_data(__('From','furniture-rental-for-woocommerce'),$values['phive_book_from_date']);
			$item->add_meta_data(__('Booked From','furniture-rental-for-woocommerce'),phive_booking_ajax_manager::phive_get_date_in_wp_format($values['phive_book_from_date']));
		}else{
			...
			if(array_key_exists('phive_book_to_date', $values)){
				$item->add_meta_data(__('To','furniture-rental-for-woocommerce'),$values['phive_book_to_date']);
				$item->add_meta_data(__('Booked To','furniture-rental-for-woocommerce'),phive_booking_ajax_manager::phive_get_date_in_wp_format($values['phive_book_to_date']));
			}
		}

		if(array_key_exists('phive_booked_price', $values)){
			$item->add_meta_data(__('Cost','furniture-rental-for-woocommerce'),$values['phive_booked_price']);
		}
